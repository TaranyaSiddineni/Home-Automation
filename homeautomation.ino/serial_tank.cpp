...
extern unsigned int volume_water;
...
unsigned int volume(void)
{
  ...
  while(!Serial.available());
  value_l = Serial.read();
  volume_water = value_h << 8 | value_l;
...
{
  Serial.write(INLET_VALVE);//start filling the tank
  Serial.write(ENABLE);
...
{  
  Serial.write(OUTLET_VALVE);//start filling the tank
  Serial.write(ENABLE);   
}
